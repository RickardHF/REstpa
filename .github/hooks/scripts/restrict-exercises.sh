...
# Block if any argument in the payload references the exercises directory.
if echo "$input" | grep -qiE 'exercises/'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Access to the exercises/ folder is restricted. Agents are not allowed to read files from this directory."}}'
    exit 0
fi
